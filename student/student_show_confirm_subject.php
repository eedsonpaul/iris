...
print "<tr>	
 ...
 $result=mysql_query("SELECT course_code,section_label,status from student_status where student_number='$student_number'");	 
 $sub= mysql_num_rows($result);
	...
	echo "<table>";
		...
		for($i = 0;$i < $index; $i++){
			...
			echo "<td align='center' width='150'>" . $status[$i] . "</td>";
		
				
			
			if($status[$i]=="confirmed"){
			echo '<td><a href="student_unconfirm_subject.php?subjectid='.$course_code[$i].'&sectionlabel='.$section_label[$i].'"> UNCONFIRM </a> </td>';
			}
			else if($status[$i]=="unconfirmed"){
			echo '<td><a href="student_confirm_subject.php?subjectid='.$course_code[$i].'&sectionlabel='.$section_label[$i].'"> CONFIRM </a> </td>';
			}
			else if($status[$i]=="waitlisted"){
				echo "<td>-------</td>";
			}
			else{
			}
